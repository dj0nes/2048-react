...
//   ReactDOM.render(<BoardUtil board_dimensions={{x:3, y:3, z:3}}/>, div);
...
it('cleans sequence of removable tokens and updates values', () => {
    ...
    let tile0 = BoardUtil.createTile({value: 0, id: 0, merged_to: 4})
    ...
    let location = {x: 0}
    let sequence = BoardUtil.getSequence(board_map, board_dimensions, direction, location)
...
describe('sequence iteration in 2D', () => {
    ...
    let tile0 = BoardUtil.createTile({value: 0, location: {x:0, y:0}, id: 0})
    let tile1 = BoardUtil.createTile({value: 0, location: {x:0, y:1}, id: 1})
    let tile2 = BoardUtil.createTile({value: 0, location: {x:0, y:2}, id: 2})
    let kv_pairs = [
        [tile0_coordinates, tile0],
        [tile1_coordinates, tile1],
    ...
    it('returns the ascending sequence of tiles on a 2D board_map in the correct order', () => {
        ...
        expect(third).toEqual([tile2_coordinates, [tile2]])
...
describe('sequence merging in 2D', () => {
    ...
    let board_dimensions = {x: 4, y: 4}
    let board_map = new BoardMap(kv_pairs)
    let tokens = [2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048]
    let transitions = [4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048]
    ...
    it('merges [0, 0, 0, 0] on a 2D board_map into [x, x, x, x]', () => {
        ...
        let merged_sequence = BoardUtil.mergeLocation(board_map, board_dimensions, direction, location, tokens, transitions)
    ...
    it('merges [2, 2, 2, 2] on a 2D board_map into [4, 4, x, x], and again into [8, 0, 0, 0]', () => {
        ...
        let merged_sequence2 = BoardUtil.mergeSequence(merged_sequence, tokens, transitions)
...
describe('board merging in 2D', () => {
    ...
    it('merges [2, 0, 0, 0] on a 2D board_map into [2, x, x, x]', () => {
        ...
        let merged_board = BoardUtil.mergeBoard(board_map, board_dimensions, direction, tokens, transitions)
